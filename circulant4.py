...
# --- COMPASS SETTINGS ---
compass_horizon = 5    # Predict average of next N tokens
compass_alpha = 0.5    # Weight of the compass loss
...
class HierarchicalGPT(nn.Module):
    def __init__(self, vocab_size):
        ...
        # --- COMPASS HEAD ---
        # Predicts the direction of the next 5 tokens' average
        self.compass_head = nn.Linear(dim_worker, dim_worker, bias=False)
        self.compass_horizon = compass_horizon
        self.compass_loss_weight = compass_alpha
    ...
    def forward(self, idx, targets=None, use_cache=True):
        ...
        x = self.w_ln_f(x) # x is (B*Num_Blocks, Local, Dim)
        
        # Reshape logits to (B, Total_T, Vocab)
        ...
        if targets is not None:
            ...
            # --- 2. COMPASS: Future Average Prediction ---
            # Create "Ground Truth" Compass Vector: Average of next 5 target embeddings
            with torch.no_grad():
                target_emb = self.w_token_emb(targets) # (B, T, dim_worker)
                target_emb_p = target_emb.transpose(1, 2) # (B, dim, T)
                
                # Pad end to handle window at boundary
                pad_amt = self.compass_horizon - 1
                target_padded = F.pad(target_emb_p, (0, pad_amt))
                
                # Sliding Window Average
                future_avgs = F.avg_pool1d(target_padded, kernel_size=self.compass_horizon, stride=1)
                future_avgs = future_avgs.transpose(1, 2) # Back to (B, T, dim)

            # Predict Compass Vector from current hidden state
            compass_pred = self.compass_head(x) 
            
            # --- FIX: RESHAPE COMPASS PRED TO MATCH TARGETS ---
            # x is (B * Num_Blocks, Local_Block_Size, Dim)
            # We need to reshape it to (B, Total_T, Dim) to match future_avgs
            compass_pred = compass_pred.view(B, Total_T, -1)

            # Loss: Cosine Similarity (Maximize alignment with future semantic center)
            # 1 - CosineSimilarity = Loss
            compass_sim = F.cosine_similarity(compass_pred, future_avgs, dim=-1)
            compass_loss = 1.0 - compass_sim.mean()

            # Combined Loss
            loss = lm_loss + vq_loss + (coherence_alpha * coherence_loss) + (self.compass_loss_weight * compass_loss)
...
if __name__ == '__main__':
    ...
    print(f"Running COMPASS-ENHANCED Hierarchical GPT on {device}")
    ...
    if os.path.exists(ckpt_path):
            ...
            try:
                ...
                scaler = torch.amp.GradScaler() 
                print("!!! Scaler force-reset to prevent dead training !!!")
